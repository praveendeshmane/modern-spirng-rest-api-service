...
try {
    ...
    "hello from quick smoke with start $(Get-Date -Format o)" | Out-File -FilePath $SampleFilePath -Encoding utf8
    ...
    $reportResp = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/v1/ops/reports/users" -Headers $authHeaders
    $reportResp | ConvertTo-Json -Depth 5 | Out-Host

    Write-Host "`n== 6) Messaging endpoint =="
    $msgBody = @{ message = "hello from api quick-smoke-with-start" } | ConvertTo-Json
    $msgResp = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/v1/ops/messages" -Headers $authHeaders -ContentType "application/json" -Body $msgBody
    $msgResp | ConvertTo-Json -Depth 5 | Out-Host
    ...
    if (-not $filename) { throw "Filename missing in upload response." }
    Write-Host "Uploaded filename: $filename"
    ...
    Write-Host "Downloaded to: $DownloadedFilePath"
    Get-Content $DownloadedFilePath | Out-Host
    ...
    Write-Host "`n✅ Full smoke (with startup) completed successfully."
    Write-Host "App is still running in background (PID $($appProc.Id))."
    Write-Host "To stop app: Stop-Process -Id $($appProc.Id) -Force"
}
finally {
    Pop-Location
}
